# This file was source from https://github.com/MzeroMiko/VMamba
# License:  MIT license
...
def cross_merge_fwd(y: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0):
    if out_channel_first: # out_channel_first = False
        B, K, D, H, W = y.shape
        y = y.view(B, K, D, -1)
        if scans == 0:
            ...
            y = y[:, 0:2] + y[:, 2:4].flip(dims=[-1]).view(B, 2, D, -1)
            y = y.sum(1)
    else: # goes on this branch
        B, H, W, K, D = y.shape
        y = y.view(B, -1, K, D) # y: [16, 459, 4, 64]
        if scans == 0: # goes on this branch, scans = 0
    ...
    if in_channel_first and (not out_channel_first): # does not enter
        y = y.permute(0, 2, 1).contiguous()
    elif (not in_channel_first) and out_channel_first: # does not enter
        y = y.permute(0, 2, 1).contiguous()

    return y # y: [16, 459, 64]
