# This file is a modified version of https://github.com/TaatiTeam/MotionAGFormer/blob/master/train_3dhp.py
# License: Apache 2.0
...
def parse_args():
    ...
    parser.add_argument('--new-checkpoint', type=str, metavar='PATH', default='checkpoints/new_checkpoint',
...
def display_weight_statistics(model):
    for name, param in model.named_parameters():
        if param.requires_grad:
            print(
                f"{name}: mean={param.mean().item()}, std={param.std().item()}, min={param.min().item()}, max={param.max().item()}")
...
def train(args, opts):
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        # display_weight_statistics(model)
        with torch.no_grad():
